...
{% macro test_spanner_google_metadata_query() %}
  ...
  {% do dbt_unittest.assert_true('spanner_type' in normalized) %}
  {% do dbt_unittest.assert_true("table_catalog = ''" in normalized) %}
  {% do dbt_unittest.assert_true("table_schema = ''" in normalized) %}
  {% do dbt_unittest.assert_true("table_name = 'Orders'" in normalized) %}
{% endmacro %}
...
{% macro test_spanner_google_sql_rendering() %}
  {% do dbt_unittest.assert_equals(
    dbt_bigquery_federation._federation_provider_render_remote_relation('spanner_google_sql', '', 'Orders'),
    '`Orders`'
  ) %}
  {% do dbt_unittest.assert_equals(
    dbt_bigquery_federation._federation_provider_render_remote_relation('spanner_google_sql', 'sales', 'Orders'),
    '`sales`.`Orders`'
  ) %}
  {% set rendered = dbt_bigquery_federation._federation_render_external_query(
    'projects/p/locations/us/connections/spanner',
    'select * from `Orders`',
    none,
    'low'
  ) %}
  {% do dbt_unittest.assert_true('query_execution_priority' in rendered) %}
  {% do dbt_unittest.assert_true('low' in rendered) %}
{% endmacro %}
...
{% macro test_spanner_google_live_plan_native_types() %}
  {% set conn = {
    ...
    'alias': 'spanner_app',
    'connection_id': 'projects/p/locations/us/connections/spanner',
    'policy': 'safe',
