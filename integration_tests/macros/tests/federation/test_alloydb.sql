{% macro test_alloydb_postgres_plan_uses_shared_profile() %}
  ...
  {% do dbt_unittest.assert_equals(
    ...
    'select "id", cast("user_uuid" as text) as "user_uuid", cast("payload" as text) as "payload" from "public"."orders"'
  ) %}
{% endmacro %}

{% macro test_alloydb_postgres_router_matches_postgres_dialect() %}
  {% do dbt_unittest.assert_equals(
    dbt_bigquery_federation._federation_provider_quote_identifier('alloydb_postgres', 'Orders'),
    '"Orders"'
  ) %}
