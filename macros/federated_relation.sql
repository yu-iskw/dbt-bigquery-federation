...
{% macro default__federated_relation(connection, table, schema=None, type_policy=None, overrides=None) -%}
  {% set result = _federation_try_plan(connection, table, schema, type_policy, overrides) %}
  {% if not result.ok %}
    {{ exceptions.raise_compiler_error(result.error) }}
  {% endif %}
  {{ return(_render_external_query(result.plan.connection_id, result.plan.remote_sql, result.plan.decimal_option)) }}
{%- endmacro %}
